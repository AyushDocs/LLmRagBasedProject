...
def total_margin_theta(data: dict):
    ...
    df = df[
        (df["Product"].str.contains("Theta", case=False))
        & (df["Country"].str.contains("IN", case=False))
    ]
...
def pizza_sales_jakarta(data: dict):
    ...
    process = subprocess.Popen(
        ["./refine"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    response = requests.post(
        f"{OPENREFINE_URL}/command/core/create-project-from-upload",
        files=files,
        data=data,
    )
    ...
    export_url = (
        f"{OPENREFINE_URL}/command/core/export-rows?project={project_id}&format=csv"
    )
...
def total_sales(data: dict):
    ...
    with open(data["file_path"], "r") as file:
        for line in file:
            sales += int(
                line[line.index('"sales":') : line.index(",", line.index('"sales":'))]
            )
    return sales
...
# Task 9: Transcribe a YouTube video spegment
def transcribe_video_segment(data: dict):
    audio_file_url = data["youtube_url"]
    start_time = data["start_time"]
    end_time = data["end_time"]

    video_id = audio_file_url.split("=")[-1]  # Fix: Support both ?v= and short URLs
    ...
    file_path = f"./youtube/{video_id}.mp3"
    cmd = f'yt-dlp --download-sections "*{start_time}s-{end_time}s" -o "{file_path}" -x --audio-format mp3 "{audio_file_url}"'
...
def reconstruct_image(data: dict):
    ...
    print(data["mapping_path"])
    for mapping in data["mapping_path"]:
        orig_r, orig_c, scram_r, scram_c = (
            mapping["original_row"],
            mapping["original_column"],
            mapping["scrambled_row"],
            mapping["scrambled_column"],
        )
        box = (
            scram_c * piece_size,
            scram_r * piece_size,
            (scram_c + 1) * piece_size,
            (scram_r + 1) * piece_size,
        )
        piece = image.crop(box)
        new_image.paste(piece, (orig_c * piece_size, orig_r * piece_size))

    output_path = "uploads/reconstructed.png"
